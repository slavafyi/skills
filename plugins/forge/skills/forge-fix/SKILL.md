...
5. If the cause is a missing requirement, unclear edge case, or design conflict, stop and route to `forge-spec` or the user instead of inventing behavior.
6. Add regression proof when there is a correct seam, using `references/regression-policy.md`.
7. Make the smallest root-cause fix, remove temporary probes, re-run validation, and update status/plan when meaningful.
...
- Do not turn an unspecified edge case into code without spec/user confirmation.
